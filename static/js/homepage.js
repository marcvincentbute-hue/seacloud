...
async function loadWeather() {
    const ports = ['Butuan Port', 'Magallanes'];
    
    for (const port of ports) {
        try {
            const response = await fetch(`/api/weather/${encodeURIComponent(port)}`);
            const data = await response.json();
            
            const portId = port.replace(' ', '');
            const weatherDiv = document.getElementById(`weather${portId}`);
            
            if (weatherDiv && !data.error) {
                weatherDiv.innerHTML = `
                    <div class="port-name">⛴️ ${data.port}</div>
                    <div class="temp">${data.temperature}°C</div>
                    <div class="condition">${data.condition}</div>
                    <div class="warning">💨 Wind: ${data.wind_speed} km/h | 💧 Humidity: ${data.humidity}%</div>
                `;
            } else if (weatherDiv) {
                weatherDiv.innerHTML = `
                    <div class="port-name">⚠️ ${port}</div>
                    <div class="condition">${data.message || 'Weather data unavailable'}</div>
                `;
            }
        } catch (error) {
            console.error(`Weather error for ${port}:`, error);
            const portId = port.replace(' ', '');
            const weatherDiv = document.getElementById(`weather${portId}`);
            if (weatherDiv) {
                weatherDiv.innerHTML = `
                    <div class="port-name">⚠️ ${port}</div>
                    <div class="condition">Unable to load weather</div>
                `;
            }
        }
    }
